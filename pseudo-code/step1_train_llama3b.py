...
# was 8
print("Lora 8")
lora_config = LoraConfig(task_type="SEQ_CLS",r=16, lora_alpha=16, target_modules=["q_proj", "v_proj"], lora_dropout=0.1, modules_to_save=["score"], )
...
training_args = TrainingArguments(
    output_dir=f"./3b_{time.strftime('%Y%m%d-%H%M%S')}",
...
def get_exponential_decay_scheduler(optimizer, warmup_steps, gamma=0.98, decay_every=400, base_lr=5e-4):
    def lr_lambda(current_step):
        if current_step < warmup_steps:
            return float(current_step) / float(max(1, warmup_steps))
        else:
            # Number of decay steps passed after warmup
            num_decays = (current_step - warmup_steps) // decay_every
...
trainer.lr_scheduler = get_exponential_decay_scheduler(
    trainer.optimizer, warmup_steps=4000, gamma=0.98, decay_every=400
)

# Add the custom LR scheduler stepper
trainer.add_callback(CustomLRSchedulerCallback())


trainer.train()

trainer.save_model("./context")

# Save the full model and tokenizer
model_dir = f"./llama3b_model_{time.strftime('%Y%m%d-%H%M%S')}"
